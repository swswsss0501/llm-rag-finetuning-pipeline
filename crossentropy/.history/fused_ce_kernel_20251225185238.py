...
def fused_cel_forward_kernel(
    ...
    # ================= 修复点 =================
    # 1. row_idx 是 tensor，必须转为 int64
    row_idx_64 = row_idx.to(tl.int64)
    
    # 2. stride 是标量(constexpr)，直接使用即可
    # 不要调用 .to(tl.int64)，因为 Python 整数没有这个方法
    # 这里的乘法 row_idx_64 * stride 会自动提升为 int64 计算
    stride_row_64 = stride_logits_row
    stride_col_64 = stride_logits_col
...
def fused_cel_backward_kernel(
    ...
    # Stride 不需要 .to()，直接赋值
    stride_logits_row_64 = stride_logits_row
    stride_logits_col_64 = stride_logits_col
    stride_grad_row_64 = stride_grad_input_row
    stride_grad_col_64 = stride_grad_input_col
