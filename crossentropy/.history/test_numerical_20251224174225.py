...
def test_precision_investigation():
    print("\n=== Test 3 (Deep Dive): Precision Verification ===")
    ...
    # 1. 制造完全相同的输入数据 (FP16)
    # 使用较大的值范围，更容易触发 FP16 的溢出或精度丢失
    logits_fp16 = torch.randn(N, V, device=device, dtype=torch.float16)
    target = torch.randint(0, V, (N,), device=device)
    
    print(f"Input Dtype: {logits_fp16.dtype}")
    
    # --- 选手 A: Triton (FP16 Input -> FP32 Compute) ---
    # 我们期望它也是高精度的
    criterion_tri = TritonCrossEntropyLoss()
    loss_triton = criterion_tri(logits_fp16, target)
    
    # --- 选手 B: PyTorch Native (FP16 Input) ---
    # 也就是刚才报错的那个
    criterion_torch = torch.nn.CrossEntropyLoss()
    loss_torch_fp16 = criterion_torch(logits_fp16, target)
    
    # --- 选手 C: PyTorch Gold Standard (FP32 Input) ---
    # 手动转成 float32 再喂给 PyTorch，这是理论上的"正确答案"
    loss_torch_fp32 = criterion_torch(logits_fp16.float(), target)
    
    # --- 结果对比 ---
    val_triton = loss_triton.item()
    val_torch_16 = loss_torch_fp16.item()
    val_torch_32 = loss_torch_fp32.item()
    
    print(f"\n[Results]")
    print(f"1. PyTorch (FP32) [真理标准]: {val_torch_32:.8f}")
    print(f"2. Triton  (FP16->FP32)   : {val_triton:.8f}")
    print(f"3. PyTorch (FP16)         : {val_torch_16:.8f}")
    
    print(f"\n[Differences]")
    diff_tri_vs_gold = abs(val_triton - val_torch_32)
    diff_torch16_vs_gold = abs(val_torch_16 - val_torch_32)
    
    print(f"Triton vs Gold Standard: {diff_tri_vs_gold:.8f}")
    print(f"PyTorch FP16 vs Gold   : {diff_torch16_vs_gold:.8f}")

    # 判定逻辑
    if diff_tri_vs_gold < 1e-4:
        print("\n✅ 结论: Triton 结果与 PyTorch FP32 高度一致。证明 Triton 精度更高，之前的误差是 PyTorch FP16 精度不足导致的。")
    else:
        print("\n❌ 结论: Triton 结果与 PyTorch FP32 差异巨大。Triton 算子内部逻辑有 Bug。")
